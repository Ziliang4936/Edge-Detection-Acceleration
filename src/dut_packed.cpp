// Packed variant of dut.cpp for Experiment 1 (4-pixel/word packing).
//
// This is a minimal modification of the working non-DATAFLOW dut.cpp:
//   - Header protocol: unchanged (5 input words, 3 response words).
//   - Input pixels: 4 uint8 pixels packed into one uint32 word,
//     LSB layout (p0 | p1<<8 | p2<<16 | p3<<24).
//     Number of input pixel words = ceil(W*H / 4).
//     Host zero-pads the last word if W*H is not a multiple of 4.
//   - Output pixels: same 4-pixel-per-word packing on the response payload.
//
// Only read_image() and write_thresholded_stream() are changed. Gaussian /
// Sobel / NMS / threshold are byte-identical to the baseline.
...
// Packed read: per-ROW packing. Each row sends ceil(width/4) words, LSB
// holds the earlier pixel. Final word of each row is zero-padded.
// Rationale: per-row packing keeps the HLS schedule trivial (fixed word
// boundaries per row, no carry state across rows) which yielded a working
// bitstream whereas whole-image packing did not.
void read_image_packed(DutStream& strmIn, Pixel image[kMaxImageHeight][kMaxImageWidth], int width, int height) {
    const int words_per_row = (width + 3) / 4;
...
#pragma HLS PIPELINE II=1
            const std::uint32_t word = static_cast<std::uint32_t>(strmIn.read());
            const int base = w * 4;
            const Pixel p0 = static_cast<Pixel>((word >>  0) & 0xffu);
            const Pixel p1 = static_cast<Pixel>((word >>  8) & 0xffu);
            const Pixel p2 = static_cast<Pixel>((word >> 16) & 0xffu);
            const Pixel p3 = static_cast<Pixel>((word >> 24) & 0xffu);
            if (base + 0 < width) image[y][base + 0] = p0;
            if (base + 1 < width) image[y][base + 1] = p1;
            if (base + 2 < width) image[y][base + 2] = p2;
            if (base + 3 < width) image[y][base + 3] = p3;
        }
    }
}
...
// Packed write: emit 3 header words unchanged, then emit the full thresholded
// image into a local line buffer one row at a time, then pack-and-emit 4
// pixels per word. Packing is per-ROW (not whole-image), so each row emits
// ceil(width / 4) words. Host side must match.
void write_thresholded_stream_packed(DutStream& strmOut,
    ...
    static Pixel row_buf[kMaxImageWidth];
...
fill_top_row:
    for (int x = 0; x < width; ++x) {
#pragma HLS PIPELINE II=1
        row_buf[x] = kNoEdgeLocal;
    }
pack_top_row:
    for (int w = 0; w < words_per_row; ++w) {
#pragma HLS PIPELINE II=1
        const int base = w * 4;
        const std::uint32_t p0 = static_cast<std::uint32_t>(row_buf[base + 0]) & 0xffu;
        const std::uint32_t p1 = (base + 1 < width) ? (static_cast<std::uint32_t>(row_buf[base + 1]) & 0xffu) : 0u;
        const std::uint32_t p2 = (base + 2 < width) ? (static_cast<std::uint32_t>(row_buf[base + 2]) & 0xffu) : 0u;
        const std::uint32_t p3 = (base + 3 < width) ? (static_cast<std::uint32_t>(row_buf[base + 3]) & 0xffu) : 0u;
        const std::uint32_t packed = p0 | (p1 << 8) | (p2 << 16) | (p3 << 24);
        strmOut.write(static_cast<DutWord>(packed));
    }

    // ---- middle rows: NMS + threshold ----
process_rows:
...
#pragma HLS INTERFACE axis port=strmOut
    ...
    read_image_packed(strmIn, input, width, height);
    gaussian_blur_hw(input, blurred, width, height);
    write_thresholded_stream_packed(strmOut, blurred, width, height, low_threshold, high_threshold);
}
