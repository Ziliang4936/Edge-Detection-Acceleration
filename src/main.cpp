...
int main(int argc, char* argv[]) {
    try {
        ...
        long long nms_total = 0, thresh_total = 0, hyst_total = 0;
        ...
        for (int iter = 0; iter < kBenchmarkIterations; ++iter) {
            ...
            nms_total    += std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
            thresh_total += std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
            hyst_total   += std::chrono::duration_cast<std::chrono::microseconds>(t3 - t2).count();
        }
        ...
        const double total_avg  = nms_avg + thresh_avg + hyst_avg;

        std::cout << "Image: " << options.input_path
                  << " (" << input_u8.width << "x" << input_u8.height << ")\n";
        std::cout << "--- Baseline timing (stages 3-5, avg over "
        ...
        std::cout << "  Total (stages 3-5):      " << total_avg  << " us\n";
